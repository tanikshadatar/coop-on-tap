  ...
  const projects=[
    {fc:"#4B7A00",coin:"assets/flavor-lime.png",flavor:"Key Lime",ribbon:"The Big Win",rcls:"r-blue",
     ...
     what:"Pulls every Zendesk ticket and Well alert for a machine into one concise summary, right inside Slack. The goal is to cut the time it takes to get up to speed on a machine's service history.",
     impact:"Live today in a pilot with tech support."},
    {fc:"#960606",coin:"assets/flavor-pom-blueberry.png",flavor:"Pomegranate Blueberry",ribbon:"Under the Hood",rcls:"r-red",
     title:"Call Summarizer Upgrade",
     what:"Our call summarizer turns service calls into quick write-ups. I moved it onto a newer Gemini model for better summaries, and fixed a bug that kept the recording links from working once it ran on Cloud Run.",
     impact:"Recording links <b>work in production again</b>, and the summaries read better. The old code needed a private key the server didn't have, so I switched it to sign links through Google's IAM instead."},
    {fc:"#C8102E",coin:"assets/flavor-watermelon.png",flavor:"Watermelon",ribbon:"New: Growth",rcls:"r-green",
     title:"Expansion Targeting MVP",
     what:"An early model with a confidence score that flags which expansion accounts are worth chasing first.",
     impact:"Sales starts with the <b>strongest leads</b> instead of guessing. Still early, but it's aimed straight at this year's growth goals."},
    {fc:"#FA5D05",coin:"assets/flavor-mango.png",flavor:"Peach Mango",ribbon:"Day-to-day",rcls:"r-orange",
     title:"Other data team work",
     what:"The everyday data team work that doesn't fit on a coin: pulling data out of Snowflake, and building new tables off the source tables in dbt so other teams can use it for what they need.",
     impact:"Not flashy, but it's the plumbing a lot of the rest of the work runs on.",
  ...
  function pour(){
    ...
    dcontent.innerHTML='<div class="dhead"><span class="icon"><img src="'+p.coin+'" alt=""></span><span class="dtag">'+p.ribbon+'</span></div><div class="fl">'+p.flavor+'</div><h3>'+p.title+'</h3><p>'+p.what+'</p><div class="impact">'+p.impact+'</div>'+toolsHTML+'<div class="dnav"><button class="btn btn-ghost" id="back">← Back to menu</button><button class="btn btn-blue" id="nextf">Next flavor →</button></div>';
